...
internal Void ui_begin(Gfx_Context *gfx, UI_Context *ui) {
    ...
    ui_width_push(ui, ui_size_pixels(0.0f));
    ui_height_push(ui, ui_size_pixels(0.0f));
    ui_layout_axis_push(ui, Axis2_X);

    // NOTE(simon): Build root
    V2U32 window_size = gfx_get_window_client_area(gfx);
    ui_width_next(ui, ui_size_pixels(window_size.width));
    ui_height_next(ui, ui_size_pixels(window_size.height));
