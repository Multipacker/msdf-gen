...
internal Void update(Void) {
    ...
    if (state->generate) {
        ...
        state->target_point_scale = 1.0f;

        arena_reset(state->data_arena);
        ...
        for (U64 i = 0; i < state->point_count; ++i) {
            U32 x_rand = 0;
            U32 y_rand = 0;
            os_get_entropy(&x_rand, sizeof(x_rand));
            os_get_entropy(&y_rand, sizeof(y_rand));
            state->points[i].x = ((F32) x_rand / (F32) U32_MAX) * 2.0f - 1.0f;
            state->points[i].y = ((F32) y_rand / (F32) U32_MAX) * 2.0f - 1.0f;
        }
        ...
        // NOTE(simon): Place the first index in the visited set.
        --free_count;
        swap(free[0], free[free_count], U64);
        visited[visited_count] = free[free_count];
        ++visited_count;
        ...
        while (free_count) {
            ...
            for (U64 i = 0; i < visited_count; ++i) {
                V2F32 visited_point = state->points[visited[i]];
                for (U64 j = 0; j < free_count; ++j) {
                    V2F32 free_point = state->points[free[j]];
                    F32 distance = v2f32_length_squared(v2f32_subtract(visited_point, free_point));
            ...
            state->edges[state->edge_count] = v2u64(visited[visited_index], free[free_index]);
            ++state->edge_count;

            --free_count;
            swap(free[free_index], free[free_count], U64);
            visited[visited_count] = free[free_count];
            ++visited_count;
        }
    ...
    ui_corner_radius(5.0f) {
        ui_fixed_position_next(state->ui_position);
        ...
        if (panel_input.input_flags & UI_InputFlag_LeftDragging) {
            ...
            V2F32 *start_position = ui_get_drag_data(V2F32);
            state->ui_position = v2f32_add(*start_position, ui_drag_delta());
        }
    }
    ...
    draw_transform(transform) {
        V4F32 line_color  = color_from_srgba(v4f32(0.41f, 0.4f, 0.4f, 1.0f));
        V4F32 point_color = color_from_srgba(v4f32(0.9f, 0.9f, 0.9f, 1.0f));
        for (U64 i = 0; i < state->edge_count; ++i) {
            draw_line(
                state->points[state->edges[i].x],
                state->points[state->edges[i].y],
                line_color,
        ...
        for (U64 i = 0; i < state->point_count; ++i) {
            ...
            draw_circle(
                state->points[i],
                state->point_radius / scale,
                color_from_srgba(srgba_from_hsva(hsva[state->coloring])),
                0.0f,
                1.0f / scale
            );
        }
